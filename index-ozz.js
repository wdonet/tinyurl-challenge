...
app.get('/u/:hash', function (req, res) {
  ...
  // console.log(hash);
  var original = db.once('value', function(snapshot) {
      var data = snapshot.val();
      for(var index in data) {
        if (data[index].hash == hash){
          var link = data[index].original;
          // var addHttp = link.startsWith('http://') ? '' : 'http://';
          // console.log(link.indexOf('http://'));
          var addHttp = 'http://';
          console.log(addHttp);
          console.log(link);
          console.log(addHttp + link);
